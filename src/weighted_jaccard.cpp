...
// Sparse accumulation: given the sparsity pattern from crossprod(binarise(x))
// as a dgCMatrix, fill in min_sums by iterating one output column at a time.
// For each output column cb, populate a row->position lookup, then iterate
// over all features where cb is nonzero, accumulating min(x[f,ca], x[f,cb])
// for each co-occurring item ca.  Writes to out[] are contiguous per column.
// [[Rcpp::export]]
...
// Dense accumulation with adaptive strategy:
// - Small ncomp (output fits in cache): feature-oriented loop, no extra setup
// - Large ncomp: column-oriented loop with item->features CSR for stride-1 writes
// [[Rcpp::export]]
